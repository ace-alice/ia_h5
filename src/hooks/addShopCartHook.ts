  ...
  function addShopCart(
    ...
    if (!isLogin.value) {
      return showToast({
        ...
        message: i18n.global.t('you_not_login_now'),
      })
    }
      ...
      getInfoFun({
        ...
        .then((res: any) => {
          if (+res.data.code === 1) {
            // if (
            //   betType === 'parlay'
            //   && +res.data.data.parlay_user_daily_payout
            //     < +res.data.data.parlay_money_min
            //   && +res.data.data.parlay_user_daily_payout === 0
            // ) {
            //   showToast({
            //     icon: notifyIcon,
            //     className: 'bet-toast-content bet-err',
            //     message: i18n.global.t('redLimitFull'),
            //   })
            // }
            if (
              betType === 'single'
            ...
            if (
              ...
              showToast({
                icon: notifyIcon,
                className: 'bet-toast-content bet-err',
                message: i18n.global.t('addError'),
              })
            }
          }
        })
