...
import WinIcon from '@/assets/home/status-1.png'
import FailIcon from '@/assets/home/status-2.png'

export default function championTeamPointHook(
  matchInfo: any,
) {
  const i18n = useI18n()
  ...
  const currentTeamPoint = computed(() => {
    const tempTeamPoint = oddInfoCache.value.find((team: any) => {
  ...
  const hasAdd = computed(() => {
    // console.log("hasAdd",singleIds.value)
