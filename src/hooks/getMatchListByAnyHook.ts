...
export default function getUniversalListHook(handicap: HandicapType) {
  const newCode: any = ref(handicap)
  const isEnd = handicap === 'end'
  // const betType: BetType = handicap === "parlay" ? "parlay" : "single";

  const { endFinish, noLoginCardShow, matchListTop } = storeToRefs(globalApiConfigStore())

  provide('betType', handicap === 'parlay' ? 'parlay' : 'single')

  provide('handicap', handicap)

  const { currentGameId, checkedEvenList, searchTeamName, currentHandicap }
  ...
  const matchListData: ComputedRef<MatchListType> = computed(() => {
    return matchList.value.filter((match) => {
      ...
      let hasTeam = true
      if (searchTeamName.value && match.team_name_1 && match.team_name_2) {
        const tag1 = match.team_name_1
          .toLocaleLowerCase()
          .indexOf(searchTeamName.value.toLocaleLowerCase())
        const tag2 = match.team_name_2
          .toLocaleLowerCase()
          .indexOf(searchTeamName.value.toLocaleLowerCase())
        const tag3 = match.event_name
          .toLocaleLowerCase()
          .indexOf(searchTeamName.value.toLocaleLowerCase())
        hasTeam = tag1 !== -1 || tag2 !== -1 || tag3 !== -1
      }
      return (
        handicap === 'hot'
        || ((currentDate.value === 'all'
  ...
  function getList() {
    ...
    if (finish.value || loadList.value) {
      return
    }
    loadList.value = true
    const form: any = {}
  ...
  watch(
    () => [currentDate.value, currentGameId.value, currentHandicap.value, checkedEvenList.value],
    () => {
      if (currentHandicap.value === handicap) {
        refreshList()
      }
   ...
   * @description 跳转投注详情页
   * */
  const matchDetail = (game_id: number, category_id: number, match: any) => {
    router.push({
      name: 'MatchDetail',
      query: {
        game_id,
