...
export default function getUniversalListHook(handicap: HandicapType) {
  ...
  const { endFinish, matchListTop } = storeToRefs(globalApiConfigStore())
      ...
      .then((res: any) => {
        if (+res.data.code === 1) {
          ...
          if (
            ...
            finish.value = true
            setTimeout(() => {
              window.scrollTo({ top: matchListTop.value, behavior: 'smooth' })
            }, 100)
            if (isEnd) {
  ...
  const tabChangeList = (id: any) => {
    formData.nav_code = id
    refreshList()
  }
