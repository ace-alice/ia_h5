...
export default function videoAndAnimationHook(matchInfo: any) {
  ...
  const animateSrc = computed(() => {
    if (animationList.value.length > 0 && animationList.value[0].has_push_pc) {
      return animationList.value[0].has_push_pc
    }
    else {
      return matchInfo.has_push_url_m
    }
  })
