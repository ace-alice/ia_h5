...
export function socketServiceHook(proxy?: any) {
   ...
   * */
  const { pushOddInfo, pushVideoInfo, pushAnimationInfo }
    = socketCacheStore()

  const { foreGroundTabCount } = gameInfoStore()
  ...
  function live_video_url_change(content: any) {
    if (content.game_id && content.has_push_m) {
      pushAnimationInfo({ game_id: Number(content.game_id), has_push_pc: content.has_push_m })
    }
    if (content.game_id && content.media_content) {
      pushVideoInfo({ game_id: Number(content.game_id), url: content.media_content })
    }
  }
  /***
   * @description 单边锁
