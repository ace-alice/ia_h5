 ...
 * @returns
 */
export function toFixedNumber(num: any, keep = fixedKeep): number {
...
export function setItemName(value: number, lang?: string) {
  if (lang) {
    return { val: value, label: lang }
  }
  switch (value) {
    case 0:
      return { val: 0, label: i18n.global.t('head_game') }
    default:
      return {
        val: value,
...
export function getBetListStatus(v: any) {
  if (+v.is_cancel === 1) {
    ...
    return { code: 2, color: '#E24F35', name: i18n.global.t('betError') }
  }
  ...
  if (/(W+)/.test(format)) {
    // eslint-disable-next-line no-lone-blocks
    {
      format = format.replace(
        ...
        RegExp.$1.length > 1
          ? RegExp.$1.length > 2
            ? `${i18n.global.t('week1')}${week[we]}`
            : `${i18n.global.t('week2')}${week[we]}`
          : week[we],
      )
    }
    if (/(Q+)/.test(format)) {
      format = format.replace(
        RegExp.$1,
        RegExp.$1.length === 4 ? `${i18n.global.t('No')}${quarter[qut]}${i18n.global.t('theQuarter')}` : quarter[qut],
      )
    }
    for (const k in opt) {
...
export function timeReverse(offset: number) {
  const nowDate = new Date().getTime() - offset * 1000 * 60 * 60 * 24 + 1
  return Number(String(nowDate).slice(0, 7).padEnd(13, '0'))
}

export function numberUnit(num: number) {
  if (num > 1000000) {
